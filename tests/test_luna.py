...
from luna.filenames import sanitize_component, suggested_filename
...
class LunaTests(unittest.TestCase):
    def test_scanner_is_recursive_and_ignores_unsupported(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual([p.name for p in (t.path for t in tracks)],['a.mp3','b.flac'])
            self.assertTrue(all(t.metadata_error for t in tracks))
    def test_normalization_preserves_unicode_and_numbers(self):
        self.assertEqual(normalize_text('  Björk   |  Vespertine  '),'Björk | Vespertine')
        self.assertEqual(normalize_track_number('03/12'),3)
    ...
    def test_rename_plan_is_dry_run_and_collision_aware(self):
        with tempfile.TemporaryDirectory() as tmp:
            root=Path(tmp); a=root/'old.mp3'; b=root/'01 - Artist - Song.mp3'; a.write_bytes(b'a'); b.write_bytes(b'b')
            track=Track(a,'Song','Artist','Album',1)
    ...
    def test_probable_duplicates_never_delete(self):
        with tempfile.TemporaryDirectory() as tmp:
            root=Path(tmp); a=root/'one.mp3'; b=root/'renamed.flac'; a.write_bytes(b'abc'); b.write_bytes(b'abc')
            tracks=[Track(a,'Song','Artist','Album',1,size=3),Track(b,'Song','Artist','Album',1,size=3)]
