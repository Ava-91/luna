...
class FilesystemIntegrationTests(unittest.TestCase):
    ...
    def test_scan_reads_real_recursive_audio_files(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(sorted(track.path.name for track in tracks), ["one.wav", "two.wav"])
    ...
    def test_real_filesystem_rename_apply_and_rollback(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            source, destination = root / "old.wav", root / "new.wav"
            self._wav(source)
    ...
    def test_real_filesystem_two_file_rename_cycle(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            plan = [RenamePlanItem(first, second, "change", "cycle"), RenamePlanItem(second, first, "change", "cycle")]
    ...
    def test_real_filesystem_three_file_rename_cycle(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            plan = [RenamePlanItem(paths[0], paths[1], "change", "cycle"), RenamePlanItem(paths[1], paths[2], "change", "cycle"), RenamePlanItem(paths[2], paths[0], "change", "cycle")]
            results = apply_rename_plan(plan, True, root=root)
            self.assertTrue(all(result.success for result in results))
    ...
    def test_external_symlink_is_rejected_by_rename(self):
        with tempfile.TemporaryDirectory() as tmp:
            base = Path(tmp); root = base / "library"; outside = base / "outside"; root.mkdir(); outside.mkdir()
            target = outside / "song.wav"; self._wav(target); link = root / "link.wav"
            ...
            self.assertFalse(result[0].success); self.assertIn("symlink", result[0].error.lower()); self.assertTrue(link.is_symlink())

    def test_internal_symlink_is_rejected_by_rename(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp); target = root / "real.wav"; self._wav(target); link = root / "link.wav"
            try: link.symlink_to(target)
            except (OSError, NotImplementedError) as exc: self.skipTest(f"symlinks unavailable: {exc}")
            result = apply_rename_plan([RenamePlanItem(link, root / "new.wav", "change", "symlink")], True, root=root)
            self.assertFalse(result[0].success); self.assertIn("symlink", result[0].error.lower()); self.assertTrue(target.exists()); self.assertTrue(link.is_symlink())

    def test_nested_external_symlink_is_rejected_by_rename(self):
        with tempfile.TemporaryDirectory() as tmp:
            base = Path(tmp); root = base / "library"; outside = base / "outside"; root.mkdir(); outside.mkdir()
            target = outside / "song.wav"; self._wav(target); first = root / "first.wav"; second = root / "second.wav"
            try: second.symlink_to(target); first.symlink_to(second)
            except (OSError, NotImplementedError) as exc: self.skipTest(f"symlinks unavailable: {exc}")
            result = apply_rename_plan([RenamePlanItem(first, root / "new.wav", "change", "symlink")], True, root=root)
            self.assertFalse(result[0].success); self.assertIn("symlink", result[0].error.lower()); self.assertTrue(target.exists())

    def test_missing_symlink_target_is_rejected_by_rename(self):
        with tempfile.TemporaryDirectory() as tmp:
            base = Path(tmp); root = base / "library"; outside = base / "outside"; root.mkdir(); outside.mkdir()
            link = root / "link.wav"
            try: link.symlink_to(outside / "missing.wav")
            except (OSError, NotImplementedError) as exc: self.skipTest(f"symlinks unavailable: {exc}")
            result = apply_rename_plan([RenamePlanItem(link, root / "new.wav", "change", "symlink")], True, root=root)
            self.assertFalse(result[0].success); self.assertIn("symlink", result[0].error.lower()); self.assertTrue(link.is_symlink())
    ...
    def test_normal_file_rename_still_works_with_boundary(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp); source = root / "old.wav"; destination = root / "new.wav"; self._wav(source)
            result = apply_rename_plan([RenamePlanItem(source, destination, "change", "normal")], True, root=root)
            self.assertTrue(result[0].success); self.assertTrue(destination.exists()); self.assertFalse(source.exists())
