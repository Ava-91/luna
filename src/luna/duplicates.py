...
def find_duplicates(tracks):
    ...
    for track in tracks:
        try:
            ...
            continue
    return sorted(
        (DuplicateGroup(d, tuple(sorted(items, key=lambda x: str(x.path)))) for d, items in by_hash.items() if len(items) > 1),
...
def find_probable_duplicates(tracks):
    ...
    for items in groups.values():
        if len(items) > 1:
            ...
            # Artist/title + size is a strong signal regardless of container format.
            # The format difference is useful context, not a reason to lower confidence.
            results.append(ProbableDuplicate(tuple(sorted(items, key=lambda x: str(x.path))), 0.95, tuple(reasons)))
