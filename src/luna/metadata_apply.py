...
def build_metadata_plan(tracks):
    return [MetadataChange(track.path,item.field,item.original,item.normalized) for track in tracks for item in normalize_track(track) if item.changed and item.normalized is not None]
...
def apply_metadata_plan(plan, confirm=False, log_path=None):
    ...
    for item in plan:
        try:
            ...
            if audio is None or audio.tags is None: audio.add_tags()
            audio.tags[item.field]=[item.new]; audio.save()
            if log: log.record("metadata",item.path,item.path)
