...
def render_markdown_report(report):
    formats = ", ".join(report["formats"]) or "none"
    return "\n".join([
        "# Luna library health",
        "",
        "| Metric | Value |",
        ...
        f"| Formats | {formats} |",
        f"| Metadata issues | {report['metadata_issues']} |",
        f"| Missing core metadata | {report['missing_core_metadata']} |",
        f"| Duplicate groups | {report['duplicate_groups']} |",
        f"| Estimated duplicate waste (bytes) | {report['duplicate_wasted_bytes']} |",
        f"| Missing artwork | {report['missing_artwork']} |",
        f"| Suspicious filenames | {report['suspicious_filenames']} |",
        f"| Proposed renames | {report['proposed_renames']} |",
    ])
