...
class Track:
    path: Path; title: str|None; artist: str|None; album: str|None; track_number: int|None=None; album_artist: str|None=None; disc_number: int|None=None; year: int|None=None; genre: str|None=None; format: str|None=None; size: int=0; modified: float=0.0; raw_metadata: dict[str,object]|None=None; metadata_error: str|None=None
...
def scan_library(root:Path,extensions=None,ignored_paths=None,workers=4,on_error=None,on_progress=None)->list[Track]:
    """Recursively scan supported audio files with bounded parallelism and optional progress callback."""
    ...
    with ThreadPoolExecutor(max_workers=max(1,workers)) as pool:
        for index,track in enumerate(pool.map(inspect_file,paths),1):
            tracks.append(track)
            if on_progress:on_progress(index,len(paths),track)
